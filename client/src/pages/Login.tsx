import { useLocation } from "wouter";
import { loginAsGuest } from "@/lib/userStorage";

export default function Login() {
  const [, setLocation] = useLocation();

  const handleGoogleLogin = () => {
    loginAsGuest("Invitado Google");
    setLocation("/home");
  };

  return (
    <div className="w-full min-h-screen bg-background flex flex-col items-center justify-center px-6">
      <div className="w-full max-w-xs flex flex-col items-center space-y-10">
        <div className="text-center space-y-2">
          <h1 className="text-3xl font-bold text-foreground" style={{ fontFamily: "Poppins" }}>
            Tu PAES <span className="text-primary">IA</span>
          </h1>
          <p className="text-[10px] text-muted-foreground/60 tracking-[0.2em] uppercase">
            Creado por Manu
          </p>
          <p className="text-xs text-muted-foreground/80 pt-1">
            Preguntas elaboradas con inteligencia artificial
          </p>
        </div>
        <button
          onClick={handleGoogleLogin}
          className="w-full h-14 bg-white hover:bg-gray-100 rounded-2xl flex items-center justify-center gap-3"
        >
          <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none">
            <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
            <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
            <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
            <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
          </svg>
          <span style={{ color: "#1f2937" }} className="font-bold text-base">Entrar con Google</span>
        </button>
      </div>
      <p className="absolute bottom-4 text-[10px] text-muted-foreground/50">
        © Tu PAES IA · Todos los derechos reservados
      </p>
    </div>
  );
}